...
import sys
import time
from importlib.resources import as_file  # 新增导入
...
# 保持原有的导入不变
from .prompt_templates import get_system_prompt, get_user_prompt
...
def run_analysis(epub_filename: str, role: str = "developer"):
    # 1. 动态构建路径
    epub_path_traversable = ASSETS_DIR / epub_filename
    with as_file(epub_path_traversable) as epub_path:  # 使用 as_file
        ...
        # 【关键】根据角色创建独立的知识库目录
        role_zh = "管理" if role == "management" else "牛马"
        output_folder_name = f"{safe_book_title}【{role_zh}】"
        current_kb_dir = OUTPUT_DIR / output_folder_name
        ...
        print(f"👤 当前角色: {role_zh}")
        print(f"📂 输出目录: {current_kb_dir}")

        # 2. 加载数据
        current_system_prompt = get_system_prompt(book_title, role=role)
        # 【核心调用】使用新的加载器，获取带层级（卷、时期）和日期的数据
        chapters = read_epub_chapters_mao_selected(
            epub_path,
            excluded_titles={"《毛主席教我们当省委书记【牛马】》", "序", "后记"},
        )
        # 加载针对这本书的记忆文件
        ...
        for i, chap in enumerate(chapters):
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        tags_display = ", ".join(
                            [f"`{t.strip()}`" for t in tags_clean.split(",")][:3]
                        )
                        # 构建索引行
            ...
            # 渲染 User Prompt
            prompt = get_user_prompt(chap["content"], context_str, role=role)
