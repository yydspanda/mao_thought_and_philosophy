...
BOOK_DIR = Path("output/毛主席教我们当省委书记【牛马】")
# ==========================================================
...
def clean_json_memory(deleted_filenames):
    ...
    for concept_name, links in appearances.items():
        ...
        appearances[concept_name] = [
            link for link in links if link not in purge_link_names
        ]
        ...
        if not appearances[concept_name]:
            ...
            print(
                f"      - 发现孤儿概念: [{concept_name}] 已没有任何引用，将被彻底删除。"
            )
...
def main():
    ...
    print(
        "2. 重新运行你的主程序 `main.py`，它会自动跳过已存在的旧文件，并为你重新生成被删除的文件（这次会是带书名号的好版本）。"
    )
    print("3. 最后，重新生成一次概念卡片以确保链接正确无误。")


# %%
# 我们把这部分代码作为一个单元格来单独调试
deleted_files = clean_duplicate_files()
print("被删除的文件列表:")
print(deleted_files)
